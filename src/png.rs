...
impl Png {
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        let index = self
            .chunks
            .iter()
            .position(|chunk| chunk == chunk_found)
            .unwrap();
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let mut bytes: Vec<u8> = Vec::from(self.header);
        self.chunks
            .iter()
...
#[derive(Debug)]
pub struct InvalidChunkHeader;

impl Display for InvalidChunkHeader {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "Chunk header is not valid")
    }
}

impl std::error::Error for InvalidChunkHeader {}

#[derive(Debug)]
pub struct InvalidChunkLenght;

impl Display for InvalidChunkLenght {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "Chunk header is not valid")
    }
}

impl std::error::Error for InvalidChunkLenght {}
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        ...
        while i < bytes.len() {
            let chunk_len = u32::from_be_bytes(
                bytes
                    .get(i..i + 4)
                    .ok_or_else(|| Box::new(InvalidChunkLenght))?
                    .try_into()?,
            ) + 12;
            let chunk_bytes = bytes
                .get(i..i + chunk_len as usize)
                .ok_or_else(|| Box::new(InvalidChunkLenght))?;
        ...
        if header == Png::STANDARD_HEADER {
            ...
            Err(Box::new(InvalidChunkHeader))
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn ttest_png_from_image_fileest_png_from_image_file() {
